...
class backupmysql {
  ...
  public function getListTables(){
    ...
    if(!$this->error){
      ...
      $re['f'] = str_ireplace('<div class="ch_tables"></div>', '', $re['f']) .'
      <input type="submit" class="btn btn-outline-dark" value="'. $this->langTxt('msg_backup') .'">
...
SET AUTOCOMMIT = 1; '. PHP_EOL;
  ...
  public function getListZip(){
    ...
    $btn = ($nr >0) ? '
    <input type="button" value="'. $this->langTxt('msg_restore_bk') .'" id="res_file"> 
    <input type="button" value="'. $this->langTxt('msg_get_file') .'" id="get_file"> 
    <input type="button" value="'. $this->langTxt('msg_del_file') .'" id="del_file">' : $this->langTxt('msg_nozip');
